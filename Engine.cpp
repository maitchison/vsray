...
void initScene(void)
{
	camera.fov = 90;
	camera.scene = &scene;
	camera.location = glm::vec3(0, 10, 30);
	camera.rotation.x = 0;	

	/*
	//scene.AddLight(new Light(glm::vec3(30, 30, 10), Color(1, 1, 1)));
	...
	sphere1 = new Sphere(glm::vec3(0, 5, -10), 5);
	sphere1->color = Color(1.0f, 0.5, 0.5);
	scene.AddEntity(sphere1);	

	sphere2 = new Sphere(glm::vec3(-15, 5, -15), 5);
	sphere2->color = Color(0.5, 1.0f, 0.5);
	scene.AddEntity(sphere2);

	sphere3 = new Sphere(glm::vec3(12, 3, -7), 3);
	sphere3->color = Color(0.5, 0.5, 1.0f);
	scene.AddEntity(sphere3);

	sphere4 = new Sphere(glm::vec3(12, 3, -30), 8);
	sphere4->color = Color(0.5, 0.5, 1.0f);
	scene.AddEntity(sphere4);

	Plane* plane1 = new Plane(glm::vec3(0,0,0));	
	scene.AddEntity(plane1);

	Plane* plane2 = new Plane(glm::vec3(0, 15, 0));
	scene.AddEntity(plane2);
	plane2->normal = glm::vec3(0, -1, 0);

	Plane* plane3 = new Plane(glm::vec3(0, 0, 20));
	scene.AddEntity(plane3);
	plane3->normal = glm::vec3(0, 0, -1);
	...
	*/
	...
	Plane* plane1 = new Plane(glm::vec3(0, 0, -20));
	plane1->normal = glm::vec3(0, 0, 1);
	scene.AddEntity(plane1);

	Plane* plane2 = new Plane(glm::vec3(-20, 0, 0));
	plane2->normal = glm::vec3(1, 0, 0);
	plane2->material->col = Color(1, 0, 0);
	scene.AddEntity(plane2);

	Plane* plane3 = new Plane(glm::vec3(+20, 0, 0));
	plane3->normal = glm::vec3(-1, 0, 0);
	plane3->material->col = Color(0, 0, 1);
	scene.AddEntity(plane3);
	
	Plane* plane4 = new Plane(glm::vec3(0, 20, 0));
	plane4->normal = glm::vec3(0, -1, 0);
	scene.AddEntity(plane4);
	
	Plane* plane5 = new Plane(glm::vec3(0, -20, 0));
	plane5->normal = glm::vec3(0, 1, 0);
	scene.AddEntity(plane5);
	
	Sphere* sphere = new Sphere(glm::vec3(0, 20, 0), 5);
	sphere->color = Color(0.5, 0.5, 1.0f);
	sphere->material->emissiveColor = Color(1, 1, 1) * 20;
	scene.AddEntity(sphere);

	sphere = new Sphere(glm::vec3(0, 00, 0), 10);
	sphere->color = Color(0.5, 0.5, 1.0f);
